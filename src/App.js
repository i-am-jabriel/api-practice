...
function App() {
  ...
  // When a user first hits my site load the list of dog breeds
  useEffect(()=>{
    fetch('https://dog.ceo/api/breeds/list/all')
      .then(res => res.json())
      .then(res => setDogBreeds(Object.keys(res.message)));
  }, [])
  ...
  return (
    <div className="App">
      ...
      <button onClick={fetchThreeDogPhotos}>Fetch 3 doggos</button>
      <br></br>
      <br></br>
      <br></br>
      <br></br>
      {dogBreeds.map(breed => <button onClick={()=>fetchDogBreed(breed)} key={breed}>{breed}</button>)}
    </div>
  );
}
